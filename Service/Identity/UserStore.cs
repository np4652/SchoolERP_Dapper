...
namespace Services.Identity
{
        ...
        IUserTwoFactorRecoveryCodeStore<ApplicationUser>
    {
        ...
        public async Task AddToRoleAsync(ApplicationUser user, string roleName, CancellationToken cancellationToken)
        {
            ...
            await _dapperRepository.ExecuteAsync("insert into RoleId values('" + user.Id + "','" + roleid + "')", null, commandType: CommandType.Text);
        }
        ...
        public async Task<IdentityResult> CreateAsync(ApplicationUser user, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var res = await _dapperRepository.ExecuteAsync("AddUser",
                    new
                    {
                        param.Id,
                        param.UserId,
        ...
        {
            var sqlQuery = $"select u.*, r.RoleId [RoleId] from Users u inner join RoleId r on r.UserId = u.Id where u.NormalizedEmail='{normalizedEmail}'";
        ...
        public async Task<bool> IsInRoleAsync(ApplicationUser user, string roleName, CancellationToken cancellationToken)
        {
            bool result = false;
            int roleid = _roleManager.FindByNameAsync(roleName).Result.Id;
            string sqlQuery = @"IF (select Count(1) from RoleId(nolock) where RoleId=@roleid and UserId=@Id)>0 select 1 else select 0";
