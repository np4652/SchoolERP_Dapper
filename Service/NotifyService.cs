...
namespace Service.Notify
{
    public class NotifyService : INotifyService
    {
        ...
        public async Task<IResponse> SaveSMSEmailWhatsappNotification(SMSEmailWhatsappNotification req, int LoginID)
        {
            ...
            if (req.IsOTP)
            {
                _res = await SaveLoginOTP(req.PhoneNumber, req.OTP, req.Action);
            }
            if (_res.StatusCode==ResponseStatus.Success)
            {
                _res.StatusCode = ResponseStatus.Failed;
                ...
                if (res.UserID>0)
                {
                    ...
                    StringBuilder sb = new StringBuilder("Check OTP on Your ");
                    if (req.IsSms)
                    {
                        NotifySMS(res);
                        sb.Append("Mobile{OR}");
                    ...
                    {
                        sb.Replace("{OR}", " Or ");
                        NotifySocialALert(res);
                        sb.Append("Whatsapp");
                    }
                    sb.Replace("{OR}", "");
                    _res.StatusCode = ResponseStatus.Success;
                    _res.ResponseText = req.IsOTP? sb.ToString(): ResponseStatus.Success.ToString();
                }
            }
            return _res;
        }
        ...
        public async Task<Response> NotifySMS(AlertReplacementModel model)
        {
            ...
            try
            {
                ...
                string selectTemplate = @" SELECT * FROM MessageTemplate(nolock) WHERE FormatID=@FormatID
                                           SELECT top(1) id, apitype,transactiontype,name, url, isactive, isdefault, isdeleted,  entryby,
                                                  entrydate, modifyby,modifydate, apimethod,restype,ismultipleallowed,ApiCode
                                           FROM   smsapi(nolock)";
    ...
    FROM   smsapi(nolock)  where id=2  ";
        ...
        private async Task<Response> SaveLoginOTP(string mobileNo, string otp, string action)
        {
            ...
            try
            {
                var sqlQuery = @"IF NOT Exists (Select 1 from Users Where PhoneNumber = @mobileNo) and @action<>'REGISTRATION'
                             begin
                             ...
                             VALUES (@mobileNo,@otp,@action,GETDATE(),0,0)
                             Select 1 StatusCode,'OTP Send Successfully' ResponseText";
                result = await _dapper.GetAsync<Response>(sqlQuery, new { mobileNo, otp, action }, commandType: CommandType.Text);
            }
            catch (Exception ex)
            {
