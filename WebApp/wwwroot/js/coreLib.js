...
const regexExpression = {
    isHTML: '/<(?=.*? .*?\/ ?>|br|hr|input|!--|wbr)[a-z]+.*?>|<([a-z]+).*?<\/\[a-z]>/i'
};
var Q;
(Q => {
    ...
    Q.confirm = (message, onYes, options) => {
        ...
        if (options.htmlEncode)
            message = `<div class="swal2-icon swal2-info swal2-icon-show" style="display: flex;"><div class="swal2-icon-content">i</div></div><div><h2>Are you sure?</h2>${Q.htmlEncode(message)}</div>`;
        var clicked = false;
        if (!options.buttons) {
            var buttons = [];
            buttons.push({
    ...
    Q.renderError = xhr => {
        if (xhr.status === 400) {
            ...
            for (var i = 0; i < validationErrors.length; i++) {
                let __span = $('small[data-valmsg-for="' + validationErrors[i].key + '"]');
                if (__span.index() == -1) {
                    console.log('[name="' + validationErrors[i].key + '"]');
                    $('[name="' + validationErrors[i].key + '"]').parent('div').append(`<small data-valmsg-for="${validationErrors[i].key}" class="text-danger">${validationErrors[i].errors[0]}</small>`);
                }
                else {
...
(function ($) {
    $.fn.fixTableHeader = function () {
        ...
        let _style = `<style>.calcHeight{height: calc(100vh - ${distance}px - 30px); }.fixedHeader th {background: #2262c6; position: sticky;top: -1px; z-index:999;border: #1f55a9 !important}</style>`
        $('head').append(_style);
